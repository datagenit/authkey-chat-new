...
const NewChatCall = () => {
  ...
  const { callData, setCallData } = ChatState();
...
const acceptChat = async()=>{
  ...
  const dataforAcceptChat = {
    ...
    name:callData.name,
    message_type:callData.message_type,
    content: callData.content,
    agent_id:currentUser.user_id
  }
  try {
    const {data} = await axios.post(`${BASE_URL2}/whatsapp_setting`,dataforAcceptChat);
    if(data.success===true){
      const dataforAcceptedChat = {
        accepted:true
      }
      socket.emit("chat accepted",dataforAcceptedChat);
  ...
  return (
    <div className="popup-box">
      <div className="popup-size">
        ...
        <div>
          <div className="mb-lg-3 pt-5 mb-2 text-center">
            <svg
              viewBox="0 0 212 212"
              height="100"
              width="100"
          ...
          <div className="row p-3">
            <div className="col-md-12 text-center">
              {callData.mobile} ({callData.name})
            </div>
            <div className="col-md-12 text-center">
              Content:{" "}
              {callData.message_type === "text"
                ? callData.content
                : callData.message_type}
            </div>
          </div>
          <div className="modal-footer">
            <button type="button" className="btn btn-primary" onClick={acceptChat}>
              Accept
            </button>
            <button
              type="button"
